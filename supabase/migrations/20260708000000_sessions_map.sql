...
alter table public.pickup_runs
  ...
  add column if not exists is_completed boolean generated always as (status = 'completed') stored;

-- ── 3. Index for map time-range queries ────────────────────────────────────
create index if not exists pickup_runs_start_at_idx
  on public.pickup_runs (start_at asc);

-- ── 4. Backfill spots_taken from live RSVPs ────────────────────────────────
...
-- Fires BEFORE INSERT | DELETE | UPDATE OF status, run_id on pickup_run_rsvps.
--
...
create trigger trg_sync_pickup_run_spots
  before insert or delete or update of status, run_id on public.pickup_run_rsvps
