    ...
    return (
      <main className="min-h-screen p-6 max-w-xl mx-auto">
        ...
        <p className="mt-3 text-gray-700">
          ...
          <br />Expected response window: 24–48 hours.
          <br />Updates live on Instagram story + the Status page.
        </p>
        ...
        <div className="mt-6 rounded-xl border p-4 space-y-2">
          ...
          {wantsTournament ? (
            <>
              <div><span className="text-gray-500">Day works:</span> {dayWorks ? "Yes" : "No"}</div>
              {!dayWorks ? <div><span className="text-gray-500">Preferred day:</span> {preferredDay || "—"}</div> : null}
              <div><span className="text-gray-500">Time choice:</span> {timeChoice}</div>
              {timeChoice === "neither" ? <div><span className="text-gray-500">Alt day/time:</span> {altTime || "—"}</div> : null}
              <div><span className="text-gray-500">Captain:</span> {captain ? "Yes" : "No"}</div>
            </>
          ) : null}
        </div>
  ...
  return (
    <main className="min-h-screen p-6 max-w-xl mx-auto">
      ...
      <p className="mt-2 text-sm text-gray-600">
        Logged in as {profile.first} {profile.last} (@{profile.ig})
      </p>
