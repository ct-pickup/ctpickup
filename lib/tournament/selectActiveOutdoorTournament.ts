...
export async function selectActiveOutdoorTournamentForRegion(
...
): Promise<{ data: OutdoorTournamentRow | null; error: { message: string } | null }> {
  ...
  if (region) {
    ...
    if (r1.data) return { data: r1.data as OutdoorTournamentRow, error: null };
  ...
  if (r0.data) return { data: r0.data as OutdoorTournamentRow, error: null };
