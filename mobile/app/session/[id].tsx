...
export default function SessionDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data: runData } = await supabase
        .from("pickup_runs")
        .select("id,title,location_text,latitude,longitude,start_at,capacity,spots_taken,fee_cents,level,open_tier_rank,run_type,status,created_by,service_region")
        .eq("id", id)
        .maybeSingle();
  ...
  const levelLabel = run.level ? LEVEL_LABELS[run.level] ?? run.level : null;
  ...
  return (
    <ScrollView style={s.root} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <View style={s.pillRow}>
        ...
        {isHost && (
          <View style={[s.pill, { borderColor: "#facc15" }]}>
            <Text style={[s.pillText, { color: "#facc15" }]}>You're hosting</Text>
          </View>
        )}
        {isJoined && !isHost && (
          <View style={[s.pill, { borderColor: LIME }]}>
            <Text style={[s.pillText, { color: LIME }]}>You're in</Text>
          </View>
        )}
      </View>
      ...
      <View style={s.card}>
        ...
        <View style={s.detailRow}>
          ...
          <Text style={s.detailText}>{run.run_type} · {tierLabel}{levelLabel ? ` · ${levelLabel}` : ""}</Text>
        </View>
        {run.fee_cents > 0 && (
          <View style={s.detailRow}>
      ...
      {!isHost && (
        <Pressable
          onPress={() => void rsvp()}
          disabled={rsvpBusy || isFull || isJoined}
      ...
      {isHost && (
        <Pressable onPress={() => void shareSession()} style={s.inviteBtn}>
          <FontAwesome name="share" size={14} color="#0a0a0a" />
      ...
      {attendees.length > 0 && (
        <>
          <Text style={s.sectionTitle}>Who's in ({attendees.length})</Text>
          <View style={s.card}>
            {attendees.map((a, i) => {
...
const s = StyleSheet.create({
  ...
  inviteBtn: { backgroundColor: LIME, borderRadius: 14, paddingVertical: 16, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 10, marginBottom: 12 },
